    ...
    public class ReservationController {
        ...
        // ReservationController.java 에 추가

        @GetMapping("/current")
        @Operation(summary = "현재 활성화된 예약 조회")
        public ResponseEntity<ApiResponse<ReservationDto>> getCurrent(Authentication authentication) { // 📍 반환 타입을 ReservationDto로 변경!
            ...
            return ResponseEntity.ok(new ApiResponse<>(
                    true,
                    currentDto != null ? "조회 성공" : "진행 중인 예약 없음",
                    currentDto,
                    0, 0
            ));
        }
